...
$(function() {
    ...
    $('#manager_form_submit_button').click(function(e) {
        ...
        for(manager in managers) {
            managers_str += manager + "=" + managers[manager] + '&';
        }
    ...
    $('#setting_form_submit_button').click(function(e) {
        ...
        let units_str = getUnits();
        $('<input type="text" name="units" value="' + units_str + '" hidden>').appendTo('#setting_form');
...
function getManagers()
{
    ...
    $('.manager').each(function() {
        if ($(this).find('.add_manager').length < 1) {
            ...
            managers[manager_name] = manager_pwd;
        }
    });
    return managers;
}
...
function getUnits()
{
    ...
    $('.unit').each(function() {
        if( $(this).find('.add_unit').length < 1) {
            units_str += $(this).find('input').attr('value') + '+';
        }
    });
    return units_str;
}
